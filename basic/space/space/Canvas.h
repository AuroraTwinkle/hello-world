...
public:
	...
	int addOneToSomeNumber(int number, int number2);//若number1不能被number2整除，则其相除结果加一
	...
	bool distLessXYScalar(std::vector<double> distCenterToSides);//判断是否有圆心到矩形四边的距离小于分割标度
	...
	std::vector<double> distCenterToSide(Rect *rect, int x,int y);//计算点到矩形四条边的距离
	//set and get method
